...
function ProductDetails() {
  ...
  const [products, setProducts] = useState([]); // array to support single or multiple products
  ...
  const fetchProductDetails = async () => {
    try {
      ...
      // Wrap single object in an array to keep map logic consistent
      setProducts(Array.isArray(data) ? data : [{ ...data, id: data.id || Date.now() }]);
    } catch (error) {
    ...
    <div className="w-full min-h-[50vh] p-10 bg-gray-200">
      {products.map((item, index) => (
        <div
          ...
          className="bg-white shadow-lg rounded-lg p-6 w-[90%] md:w-[60%] mb-6"
        >
